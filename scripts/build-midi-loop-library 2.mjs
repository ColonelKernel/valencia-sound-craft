...
const midiRoot = process.env.NATE_SMITH_MIDI_ROOT
  || "/Volumes/Mac-Storage/Samples/Nate Smith Samples";
...
function parsePackDirectory(packDir) {
  const folder = path.basename(packDir);
  const match = folder.match(/^(.+?)_(\d+)bpm$/i);

  if (!match) {
    return null;
  }

  const pathParts = packDir.split(path.sep);
  if (!pathParts.includes("MIDI Loops")) {
    return null;
  }

  const hasMidiFiles = fs.readdirSync(packDir, { withFileTypes: true })
    .some((entry) => entry.isFile() && isMidiFile(entry.name));

  if (!hasMidiFiles) {
    return null;
  }

  const [, family, bpmRaw] = match;
...
function discoverPackConfig() {
  const packs = [];
  const pendingDirs = [midiRoot];

  while (pendingDirs.length > 0) {
    const currentDir = pendingDirs.pop();
    const pack = parsePackDirectory(currentDir);
...
function main() {
  const packConfig = discoverPackConfig();
  const loops = [];

  for (const pack of packConfig) {
    const fileNames = fs.readdirSync(pack.dirPath)
      .filter((fileName) => isMidiFile(fileName))
      .sort((left, right) => left.localeCompare(right));

    for (const fileName of fileNames) {
      loops.push(quantizeLoop(path.join(pack.dirPath, fileName), pack));
    }
  }

  loops.sort(compareLoops);
  fs.writeFileSync(outputPath, buildOutputFile(loops));
  console.log(`Wrote ${loops.length} MIDI loops from ${packConfig.length} packs to ${outputPath}`);
}
